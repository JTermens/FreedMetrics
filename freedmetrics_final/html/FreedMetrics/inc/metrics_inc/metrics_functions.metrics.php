...
require "$metrics_incDir/twitter_metrics.metrics.php";

function retrive_metrics($article_data,$conn,$article_id){

	/// retrive METRICS

	#$readcount_mendeley = readcount_mendeley($doi);    #it does not work always
	$wikipedia_references = wiki_search($article_data['title']);
	if ($article_data['doi'] != ''){
		$crossref_references = crossref_references($article_data['doi']);
	}else{
		$crossref_references = '';
	}
	if ($article_data['source'] == 'pubmed'){
		$pubmed_citations = pubmed_citations($article_data['source_id']);
	}else{
		$pubmed_citations = '';
	}
	
	$total_tweets = total_tweets('"'.$article_data['title'].'"');
	$original_tweets = original_tweets('"'.$article_data['title'].'"');

	/// save them in a dict to be easily shown
	$metrics_dict = array('Wikipedia References' => $wikipedia_references,
	...
	//  TWEET UPLOAD  //
  	...
  	return array($metrics_dict, $tweet_array);
}
...
function pubmed_citations($query) {
	$search = file_get_contents('https://eutils.ncbi.nlm.nih.gov/entrez/eutils/elink.fcgi?dbfrom=pubmed&linkname=pubmed_pubmed_citedin&id='.$query.'=FreedMetrics&email=freedmetrics@gmail.com'
);
...
preg_match('/reference-count":(\d+),/',$search, $count);

return $count[1];
}
#$a = crossref_references('10.3390/ijms21041266');
#echo $a;
