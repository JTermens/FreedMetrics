...
if(mysqli_num_rows($query) == 1){
  // If this is true, then the user is at least registered, now check if the password is correct:

  $check = "SELECT * FROM Persons WHERE email='".$email."' && password='".md5($password)."'";
  $query = mysqli_query($conn, $check);
  if(mysqli_num_rows($query) == 1){
    // User is registered and the password is valid. Let's check if the account is activated

    $check = "SELECT * FROM Persons WHERE email='".$email."' && password='".md5($password)."' && is_verified=1";
    $query = mysqli_query($conn, $check);
    if(mysqli_num_rows($query) == 1){
      // User account is activated. Proceed to fetch the info for the session.
    ...
    header("location:index.php?pagename=Login&error=wrong_password");
  }
}else{
